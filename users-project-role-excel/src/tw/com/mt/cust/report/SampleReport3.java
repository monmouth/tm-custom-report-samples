...
public class SampleReport3 extends CustomReport {
    private static final Logger LOG = Logger.getLogger(SampleReport3.class);
     ...
     * 、專案主持人、團隊角色。
     */
    public void exportSampleReport(WebContext cont) {
        init();
        ...
        ExcelTemplateStrategy strategy = new ExcelTemplateStrategy(rb,
                userKeyToProjData, sortUser(keyToUser.values()));
        Workbook wb = strategy.create();
        try {
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            wb.write(out);
            Util.sendFile(cont, new ByteArrayInputStream(out.toByteArray()),
                    "report.xls", "application/vnd.ms-excel");
        } catch (IOException e) {
            LOG.warn("make excel file fail : " + e);
        }
    }
